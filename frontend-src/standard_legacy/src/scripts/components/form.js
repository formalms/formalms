...
var Form = (function() {
  ...
  function watchDuplicateInput(input) {
    var value = $(input).prop('value');
    var valueOriginal = $('[name="' + $(input).attr('data-watch') + '"]').prop(
      'value'
    );
  ...
  Form.prototype.init = function() {
    ...
    $(this.config.form)
      ...
      .on(
        'input',
        $.proxy(function(ev) {
          this.errors[$(ev.currentTarget).attr('id')] = validateEmail(
            $(ev.currentTarget)
          );
    ...
    $(this.config.form)
      ...
      .on(
        'input',
        $.proxy(function(ev) {
          this.errors[$(ev.currentTarget).attr('id')] = validateLength(
            $(ev.currentTarget),
            6
          );
          canSubmit(this.config.form, this.errors);
    ...
    $(this.config.form)
      ...
      .on(
        'input',
        $.proxy(function(ev) {
          this.errors[$(ev.currentTarget).attr('id')] = watchDuplicateInput(
            $(ev.currentTarget)
          );
          canSubmit(this.config.form, this.errors);
        }, this)
        ...
        $.proxy(function(ev) {
          var _isRequired =
            $(ev.currentTarget)
              ...
              .indexOf('required') >= 0;
          if (_isRequired) {
            this.errors[$(ev.currentTarget).attr('id')] = validateRequired(
              $(ev.currentTarget)
            );
