...
//	    echo   $html
        ...
        $path_course = 'files/appLms/'.Get::sett('pathcourse').'/';
        ...
        while($row = sql_fetch_assoc($result))
        {
         ...
         // calcola categoria di appartenenza 
        $arr_cat = $smodel->getMinorCategoryTree((int)$row['idCategory']);
        
        if($row['course_type']=="elearning") $img_type =  "<img src='templates/standard/images/lobject/scormorg.gif'>";
        if($row['course_type']=="classroom") $img_type = "<img src='templates/standard/images/course/classroom-cal.png'>";        
    ...
    // BUG TRACKER - LR #5669
        ...
        $can_enter_star = true;
        $can_enter_end = true  ;  
        if($data_inizio != "0000-00-00") $str_lock_start = "<p align=center><b><i style='font-size:.68em'>".Lang::t('_COURSE_BEGIN', 'certificate')."</b>: ".Format::date($data_inizio, 'date')." ".$str_h_begin."</i>" ;
        if($data_end  != "0000-00-00") $str_lock_end= "<br><b><i style='font-size:.68em'>".Lang::t('_COURSE_END', 'certificate')."</b>: ".Format::date($data_end, 'date')." ".$str_h_end."</i></p>";
        ...
        $html .= '
                        ...
                        <li>
                                        <div class="cbp-vm-image" >
                                                            ...
                                                            .($row['use_logo_in_courselist'] && !$row['img_course'] ? '<div class="logo_container"><img class="group list-group-image" src="templates/standard/images/course/course_nologo.png'.'" alt="'.Util::purge($row['name']).'" /></div>' : '')                         
                                                            .
                                                           '
                                        </div>
                                        '.$img_type.'
