	...
	<?php echo '<div class="dash-course ' .
		...
		<div class="logo_container">
			<img class="clogo cnologo"
				 src="<?php echo FormaLms\lib\Get::tmpl_path() . 'images/course/course_nologo.png'; ?>"
				alt="<?php echo Util::purge($course['name']); ?>" />
		</div>
		<?php } ?>
		...
		<h2>
			...
			<?php } else {
            echo FormaLms\lib\Get::img('standard/locked.png', Lang::t('_' . strtoupper($course['can_enter']['reason']), 'standard'));
		...
		<p class="course_support_info">
			...
			<?php
                if ($_date_limit) {
                    echo '&nbsp;(' . Lang::t('_UNTIL', 'standard') . ' ' . Format::date(substr($_date_limit, 0, 10), 'date') . ')';
                }
            ?>
			<?php } ?>
			<?php
