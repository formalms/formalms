...
if (!Forma::user()->isAnonymous() && $session->get('idCourse')) {
    ...
    if ($session->get('idCourse')) {
        ...
        $course_name = Forma::course()->getValue('name');
        if (Forma::course()->getValue('use_logo_in_courselist')) {
            $course_img = (Forma::course()->getValue('img_course') == '' ? FormaLms\lib\Get::tmpl_path() . 'images/course/course_nologo.png' : $path . Forma::course()->getValue('img_course'));
        }
    ...
    foreach ($menu_module as $id_main => $menu) {
        ...
        if ($cont == 1) {
            $span = '<span class="glyphicon glyphicon-inbox"></span> ';
        }
        if ($cont == 2) {
            $span = '<span class="glyphicon glyphicon-road"></span>';
        }
        if ($cont == 3) {
            $span = '<span class="glyphicon glyphicon-stats"></span> ';
        }
    ...
    if ($course_type === 'assessment' && Forma::user()->getUserLevelId() === ADMIN_GROUP_GODADMIN) {
    ...
    if ($session->get('idCourse')) {
        ...
        $sponsor_link = Forma::course()->getValue('linkSponsor');
        $sponsor_img = Forma::course()->getValue('imgSponsor');
        ...
        if (Forma::course()->getValue('show_who_online') == _SHOW_INSTMSG) {
            addCss('instmsg');
            addJs($GLOBALS['where_lms_relative'] . '/modules/instmsg/', 'instmsg.js');

            $GLOBALS['page']->add(
                '<script type="text/javascript">'
                . " setup_instmsg( '" . Forma::user()->getIdSt() . "', "
                . "'" . $userid . "', "
                . "'" . getPathImage('fw') . "' ); "
                . '</script>' . "\n", 'page_head');
            ...
            $user_stats['body'][2] = '<b id="user_online_n">'
                ...
                . '</a></b>';
        } elseif (Forma::course()->getValue('show_who_online') == _SHOW_COUNT) {
            $user_stats['head'][2] = Lang::t('_WHOIS_ONLINE', 'course');
            $user_stats['body'][2] = '<b id="user_online_n">'
                    . TrackUser::getWhoIsOnline($session->get('idCourse'))
                . '</b>';
        ...
        if ($sponsor_img != '') {
            ...
            $link_arg = Lang::t('_SPONSORED_BY', 'course');
        }
