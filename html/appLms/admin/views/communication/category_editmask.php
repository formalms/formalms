...
if (isset($id_parent)) {
    ...
    $_form_action = 'ajax.adm_server.php?r=alms/communication/add_category_action';
} else {
    $_form_id = 'mod_category_form';
    $_form_action = 'ajax.adm_server.php?r=alms/communication/mod_category_action';
}
...
if (isset($id_category)) {
    $content .= Form::getHidden('id_category', 'id_category', $id_category);
}

//if we are creating a new category, then get the id of the parent folder
if (isset($id_parent)) {
    $content .= Form::getHidden('id_parent', 'id_parent', $id_parent);
}
