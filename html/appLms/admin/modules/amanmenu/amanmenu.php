...
function editcustom($load = false)
{
    ...
    if ($load == false) {
        ...
        $duplicate = FormaLms\lib\Get::gReq('duplicate', DOTY_INT, 0);
        if ($duplicate > 0) {
        }
    } else {
...
function editmenuvoice($load = false)
{
    ...
    // Find images
    $all_images = [];
    /*$templ = dir( FormaLms\lib\Get::tmpl_path('lms').'/images/menu/');
...
function manmodule()
{
    ...
    if ($used_module != '') {
        $query_free_module .= ' AND idModule NOT IN ( ' . substr($used_module, 0, -1) . ' )';
    }
