...
class Selector_Course
{
    ...
    function loadCourseSelector($noprint = false, $with_assesment = false)
    {
        ...
        while (list($idCourse, $code, $name, $desc, $status, $difficult, $auto_sub, $end_mode, $show_rules, $max_user_sub) = sql_fetch_row($re_course)) {
            ...
            if (isset($this->current_selection[$idCourse])) unset($this->current_selection[$idCourse]);
        }
...
class Man_Course
{
     ...
     */
    function getCourseInfo($idCourse)
    {
    ...
    function saveCourse($idCourse, $course_info)
    {

        $field = [];
        $value = [];

        if (!is_array($course_info) || empty($course_info)) return $idCourse;
    ...
    function deleteCourse($idCourse)
    {
        ...
        // delete the course
        if (removeCourse($idCourse)) return true;
        return false;
    }
     ...
     */
    function &getWaitingSubscribed($idCourse, $edition_id = 0)
    {
     ...
     */
    function &getCourseIdstGroupLevel($idCourse)
    {
    ...
    function getIdUserOfLevel($idCourse, $levelNumber = false, $idEdition = false)
    {
        ...
        if ($levelNumber !== false) $query_courseuser .= " AND c.level = '" . $levelNumber . "'";
        $re_courseuser = sql_query($query_courseuser);
     ...
     */
    function getLevelsOfUsers($idCourse, $arr_users = false, $edition_id = false)
    {

        $id_users = [];
        if (!is_array($arr_users)) $arr_users = [$arr_users];
    ...
    function &getModulesName($idCourse)
    {
    ...
    function addMainToCourse($idCourse, $name)
    {
    ...
    function removeCourseRole($idCourse)
    {
    ...
    function removeCourseMenu($idCourse)
    {
    ...
    function canEnterCourse(&$course, $id_path = 0)
    {
        ...
        if ($course['level'] > 3) return ['can' => true, 'reason' => '', 'expiring_in' => $expiring];
        if (isset($course['prerequisites_satisfied']) && $course['prerequisites_satisfied'] == false) return ['can' => false, 'reason' => 'prerequisites', 'expiring_in' => $expiring];
        if (isset($course['waiting']) && $course['waiting'] >= 1) return ['can' => false, 'reason' => 'waiting', 'expiring_in' => $expiring];
        ...
        if ($course['date_begin'] != '0000-00-00') {
            ...
            if ($now < $time_start) return ['can' => false, 'reason' => 'course_date', 'expiring_in' => $expiring];
        }
        if ($course['date_end'] != '0000-00-00') {
            ...
            if ($now > $time_end) return ['can' => false, 'reason' => 'course_date', 'expiring_in' => $expiring];
        }
        ...
        if ($course['valid_time'] != '0' && $course['valid_time'] != '' && $course['date_first_access'] != '') {
            ...
            if ($now > ($time_first_access + ($course['valid_time'] * 24 * 3600))) return ['can' => true, 'reason' => 'course_valid_time', 'expiring_in' => $expiring];
        }
        if ($course['user_status'] >= 0 && $course['userStatusOp'] & (1 << $course['user_status'])) return ['can' => false, 'reason' => 'user_status', 'expiring_in' => $expiring];
        ...
        if ($course['course_status'] != 1 && $course['level'] < 4) return ['can' => true, 'reason' => 'course_status', 'expiring_in' => $expiring];
        return ['can' => true, 'reason' => '', 'expiring_in' => $expiring];
    }


    function getClassroomTeachers($idCourse)
    {
...
class Man_CourseUser
{
     ...
     */
    function &getUserCoursesLevelFilter($id_user, $levelNumber, $not_assessment = false)
    {
     ...
     */
    function isEnrolled($id_user, $idCourse)
    {
        $q = "SELECT COUNT(*)"
...
class DoceboCourse
{
    ...
    function DoceboCourse($idCourse)
    {
     ...
     * @return    array    [lv] => idst, [lv] => idst
     */
    function getCourseLevel($idCourse, $also_waiting = false)
    {
        require_once(_lms_ . '/lib/lib.subscribe.php');
     ...
     * @return    array    [lv] => idst, [lv] => idst
     */
    function createCourseLevel($idCourse)
    {
