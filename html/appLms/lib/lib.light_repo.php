...
class LightRepoManager
{
    ...
    public function _tableRepo()
    {
        return $GLOBALS['prefix_lms'] . '_light_repo';
    }

    public function _tableFile()
    {
        return $GLOBALS['prefix_lms'] . '_light_repo_files';
    }

    public function _tableUser()
    {
        return $GLOBALS['prefix_lms'] . '_light_repo_user';
    }

    public function getRepoList($filter_by_author = false)
    {
    ...
    public function getRepoDetails($id_repo)
    {
    ...
    public function saveRepo($id_repo, $data)
    {
        if ($id_repo == 0) {
            $keys = [];
            foreach ($data as $k => $v) {
    ...
    public function deleteRepo($id_repo)
    {
    ...
    public function getUserLastEnterInRepo($id_repo)
    {
        $query = ''
        . ' SELECT last_enter '
    ...
    public function setUserLastEnterInRepo($id_repo)
    {
        $query = ''
        . ' SELECT last_enter '
        . ' FROM ' . $this->_tableUser() . ' '
    ...
    public function getNumberOfFileInReport($id_repo, $from_date = false)
    {
        $query = '' .
                ' SELECT COUNT(*)' .
                ' FROM ' . $this->_tableFile() . '' .
    ...
    public function getNumberOfFile($group_by, $from_date = false)
    {
        $query = ' SELECT COUNT(*)'
    ...
    public function getRepoFileListOfAuthor($id_repo, $author)
    {
        $query = ' SELECT ' . implode(',', $this->_field_file) . ' '
            . ' FROM ' . $this->_tableFile()
    ...
    public function getFileInfo($arr_file)
    {
    ...
    {
        return '/appLms/repo_light/';
    }

    public function saveFile($id_file, $file_info)
    {
        require_once _base_ . '/lib/lib.upload.php';

        if ($id_file != '0') {
            $old_file = sql_fetch_row($this->getFileInfo($id_file));
            if ($old_file == false || count($old_file) == 0) {
                $old_file = false;
            }
        } else {
            $old_file = false;
        ...
        $saved_file = '';
        if ($file_info[LR_FILE_NAME] == false && $old_file == false) {
            return false;
    ...
    public function uploadFile($file_descriptor)
    {
        $file_name = '';
