...
class Man_Eportfolio
{
     ...
     */
    public function getTableEpfMember()
    {
     ...
     */
    public function getTableCompetences()
    {
     ...
     */
    public function getTableScoreCompetences()
    {
     ...
     */
    public function getTablePdpAnswer()
    {
     ...
     */
    public function getTableCompetenceInvite()
    {
     ...
     */
    public function getTablePresentation()
    {
     ...
     */
    public function getTablePresentationInvite()
    {
     ...
     */
    public function getTablePresentationAttach()
    {
     ...
     */
    public function getTableCurriculum()
    {
     ...
     */
    public function savePortfolio($id_portfolio, $array_data)
    {
     ...
     */
    public function deletePortfolio($id_portfolio)
    {
        if ($id_portfolio == 0) {
            return true;
        }
        ...
        if (!$this->deletePdpOfEportfolio($id_portfolio)) {
            return false;
        }

        if (!$this->deleteCompetenceOfEportfolio($id_portfolio)) {
            return false;
        }
     ...
     */
    public function getEportfolio($id_portfolio)
    {
        $data = [];
        if ($id_portfolio == 0) {
            return $data;
     ...
     */
    public function getAllEportfolio($filter_title = '', $ini = false, $limit = false)
    {
        $data = [];
        $query = '
		SELECT id_portfolio, title, description, custom_pdp_descr, custom_competence_descr 
     ...
     */
    public function getEportfolioInfo($arr_epf, $ini = false, $limit = false)
    {
        $data = [];
        $query = '
		SELECT id_portfolio, title, description, custom_pdp_descr, custom_competence_descr 
     ...
     */
    public function getQueryResultEportfolio($filter_title = '', $ini = false, $limit = false)
    {
        $data = [];
        $query = '
		SELECT id_portfolio, title, description, custom_pdp_descr, custom_competence_descr  
     ...
     */
    public function getTotalOfEportfolio($filter_title = '')
    {
        $number_of = 0;
        $query = '
		SELECT COUNT(*) 
     ...
     */
    public function getNumberOfAssociatedMember($arr_portfolio = false, $type_of_assoc = false)
    {
        $members = [];
        ...
        if ($type_of_assoc !== false) {
            if ($type_of_assoc == 'admin') {
                $query .= " AND user_is_admin = 'true' ";
            } else {
                $query .= " AND user_is_admin = 'false' ";
            }
        }
     ...
     */
    public function getEportfolioAssignedTo($arr_members)
    {
        $portfoli['user'] = [];
        $portfoli['admin'] = [];
        ...
        while (list($id_p, $user_is_admin) = sql_fetch_row($re_members)) {
            if ($user_is_admin == 'false') {
                $portfoli['user'][$id_p] = $id_p;
            } else {
                $portfoli['admin'][$id_p] = $id_p;
            }
        }
     ...
     */
    public function &getAssociatedMember($id_portfolio)
    {
        $members = [];
        $re_members = $this->_query('
		SELECT idst_member 
     ...
     */
    public function &getAssociatedAdmin($id_portfolio)
    {
        $members = [];
        $re_members = $this->_query('
		SELECT idst_member
     ...
     */
    public function addMembers($id_portfolio, $members_to_add, $user_is_admin = false)
    {
        if (!is_array($members_to_add)) {
            return false;
        }
     ...
     * @return bool the operation result
     */
    public function updateMembers($id_portfolio, $members_to_add, $user_is_admin = false)
    {
        if (!is_array($members_to_add)) {
            return false;
        }
     ...
     * @return bool the operation result
     */
    public function removeMembers($id_portfolio, $members_to_remove, $user_is_admin = false)
    {
        if (!is_array($members_to_remove)) {
            return false;
        }
     ...
     */
    public function getCurriculumPath()
    {
     ...
     */
    public function getCurriculum($id_portfolio, $id_user)
    {
        $query = '
		...
		WHERE id_portfolio = '" . $id_portfolio . "' 
			AND id_user = '" . $id_user . "' ";
        if (!$re_curriculum = $this->_query($query)) {
            return false;
        }
        if (sql_num_rows($re_curriculum) == 0) {
            return false;
        }
    ...
    public function saveCurriculum($id_portfolio, $id_user, $file_descriptor)
    {
        $curriculum_file = '';
    ...
    public function delCurriculum($id_portfolio, $id_user)
    {
        $sel_query = '
		SELECT curriculum_file 
		FROM ' . $this->getTableCurriculum() . " 
     ...
     */
    public function getPdpDetails($id_pdp)
    {
        $data = [];
        $query = '
     ...
     */
    public function getQueryPdpOfEportfolio($id_portfolio, $from = false, $for = false)
    {
        $data = [];
        $query = '
		SELECT id_pdp, textof, allow_answer, max_answer, answer_mod_for_day, sequence, id_portfolio 
     ...
     */
    public function getTotalOfPdp($id_portfolio)
    {
        $number_of = 0;
        $query = '
		SELECT COUNT(*) 
     ...
     */
    public function getNextPdpSequence($id_portfolio)
    {
        $data = [];
        $query = '
		SELECT MAX(sequence) 
     ...
     * @return int the id of the eportfolio saved (the new id if created) or false if fail
     */
    public function savePdpQuestion($id_pdp, $id_portfolio, $array_data)
    {
        if ($id_pdp === false) {
     ...
     * @return bool true if success, false otherwise
     */
    public function movePdp($direction, $id_pdp, $id_portfolio)
    {
     ...
     * @return bool true if success, false otherwise
     */
    public function deletePdpQuestion($id_pdp)
    {
        if ($id_pdp == 0) {
            return true;
        }
     ...
     * @return bool true if success, false otherwise
     */
    public function deletePdpOfEportfolio($id_portfolio)
    {
        $query = '
		SELECT id_pdp 
		FROM ' . $this->getTablePdp() . " 
     ...
     * @return resource_id all the pdp question found
     */
    public function getQueryPdpUserAnswer($id_pdp, $id_user)
    {
        $data = [];
        $query = '
     ...
     */
    public function getCountPdpUserAnswer($id_pdp, $id_user)
    {
        $data = [];
        $query = '
		SELECT COUNT(*)
     ...
     */
    public function getPdpAnswer($id_answer, $id_user)
    {
        $data = [];
        $query = '
		SELECT id_answer, id_user, id_pdp, textof, post_date 
     ...
     */
    public function savePdpAnswer($id_answer, $id_pdp, $id_user, $textof)
    {
        if ($id_answer == 0) {
            // insert a new one
            $query = '
     ...
     */
    public function getCompetenceDetails($id_competence)
    {
        $data = [];
        $query = '
        ...
        if (!$re_competence = $this->_query($query)) {
            return $data;
        }
     ...
     */
    public function getQueryCompetenceOfEportfolio($id_portfolio, $from = '', $for = '')
    {
        $data = [];
        $query = '
     ...
     */
    public function getTotalOfCompetence($id_portfolio)
    {
        $number_of = 0;
        $query = '
		SELECT COUNT(*) 
     ...
     */
    public function getNextCompetenceSequence($id_portfolio)
    {
        $data = [];
        $query = '
		SELECT MAX(sequence) 
     ...
     */
    public function moveCompetence($direction, $id_competence, $id_portfolio)
    {
        return utilMoveItem($direction,
     ...
     */
    public function saveCompeteceQuestion($id_competence, $id_portfolio, $array_data, $block_competence = 0)
    {
     ...
     */
    public function modBlockCompeteceQuestion($id_competence, $block_competence)
    {
        $query = '
		UPDATE ' . $this->getTableCompetences() . " 
     ...
     * @return bool true if success, false otherwise
     */
    public function deleteCompetenceOfEportfolio($id_portfolio)
    {
        $query = '
     ...
     */
    public function getDetailedCompetenceScore($id_portfolio, $estimated_user, $from_user = false)
    {
        $data = [];
        $query = '
     ...
     */
    public function saveAllCompetenceScore($id_portfolio, $estimated_user, $from_user, $arr_score, $arr_comment)
    {
        $re = true;
     ...
     */
    public function getCompetenceInvite($invited_user, $sender, $id_portfolio)
    {
        $query = '
		...
		WHERE invited_user = '" . $invited_user . "'
			...
			AND refused = 0";
        if (!$re_invite = $this->_query($query)) {
            return false;
        }
     ...
     */
    public function getAllCompetenceInvite($id_user, $id_portfolio = false)
    {
        $query = '
		SELECT invited_user, sender, id_portfolio, message_text, refused
		FROM ' . $this->getTableCompetenceInvite() . " 
        ...
        if ($id_portfolio !== false) {
            $query .= " AND id_portfolio = '" . $id_portfolio . "'";
        }
        if (!$re_invite = $this->_query($query)) {
            return false;
        }
     ...
     * @return resource_id the details of the invitation recived
     */
    public function getAllCompetenceRefusedInvite($id_user, $id_portfolio = false)
    {
        $query = '
		SELECT invited_user, sender, id_portfolio, message_text, refused
		FROM ' . $this->getTableCompetenceInvite() . " 
     ...
     */
    public function getCompetenceInviteNumber($id_user, $id_portfolio = false)
    {
        $query = '
		SELECT COUNT(*)
		FROM ' . $this->getTableCompetenceInvite() . " 
     ...
     * @return int the number of invitation recived
     */
    public function getUserInvtedByUser($id_user, $id_portfolio = false)
    {
        $query = '
		SELECT invited_user
		FROM ' . $this->getTableCompetenceInvite() . " 
     ...
     * @return bool true if success false otherwise
     */
    public function createCompetenceInvite($invited_user, $sender, $id_portfolio, $message_text)
    {
        $query = '
     ...
     * @return bool true if success false otherwise
     */
    public function refuseCompetenceInvite($invited_user, $sender, $id_portfolio)
    {
        $query = '
     ...
     * @return bool true if success false otherwise
     */
    public function deleteCompetenceInvite($id_portfolio, $sender, $invited_user)
    {
        $query = '
     ...
     */
    public function getQueryPresentation($id_portfolio, $id_user)
    {
        $query = '
		SELECT id_presentation, id_portfolio, title, textof, owner, show_pdp, show_competence, show_curriculum, pubblication_date
		FROM ' . $this->getTablePresentation() . " 
     ...
     */
    public function createPresentation($id_portfolio, $title, $textof, $owner, $show_pdp, $show_competence, $show_curriculum, $publish_date)
    {
        $query = '
     ...
     * @return bool true if succed, false otherwise
     */
    public function updatePresentation($id_presentation, $id_portfolio, $title, $textof, $owner, $show_pdp, $show_competence, $show_curriculum, $publish_date)
    {
        $query = '
    ...
    public function updatePresentationAttach($id_presentation, $id_user, $file_selection)
    {
    ...
    public function getPresentationAttach($id_presentation, $id_user)
    {
        $files = [];
        $query = '
		SELECT id_file
     ...
     */
    public function delPresentation($id_presentation, $id_user = false)
    {
        $query = '
		DELETE FROM ' . $this->getTablePresentation() . "  
		WHERE id_presentation = '" . $id_presentation . "'";
        if ($id_user !== false) {
            $query .= " AND owner = '" . $id_user . "'";
        }
     ...
     */
    public function savePresentationInvite($id_presentation, $email, $code, $send_date)
    {
        $query = '
     ...
     */
    public function validateInvite($id_presentation, $security_code)
    {
        $query = '
...
class EpfShowPresentation
{
     ...
     */
    public function _setPresentationDetail()
    {
     ...
     * @param Man_Eportfolio $man_epf a valid instance of the eportfolio manager class
     */
    public function changeEpfManager($man_epf)
    {
        $this->man_epf = $man_epf;
        $this->_setPresentationDetail();
    }
     ...
     * @param int $id_presentation the id of the presentation
     */
    public function changePresentation($id_presentation)
    {
        $this->id_presentation = $id_presentation;
        $this->_setPresentationDetail();
    }
    ...
    {
        $html = '';
        if ($this->details[PRES_SHOW_COMPETENCE] == '0') {
            return $html;
        }
    ...
    public function getCurriculum($ext = false, $code = false)
    {
        $html = '';
        if ($this->details[PRES_SHOW_CURRICULUM] == '0') {
            return $html;
        }
        ...
        if ($curriculum === false) {
            ...
            if ($ext) {
                $link = 'index.php?modname=eportfolio&amp;type=ext&amp;op=downloadcurriculum&amp;id_presentation=' . $this->id_presentation . '&amp;id_portfolio=' . $this->id_portfolio . '&amp;id_user=' . $this->id_user . '&amp;code=' . $code . '&amp;no_redirect=1';
            } else {
                $link = 'index.php?modname=eportfolio&amp;op=downloadcurriculum&amp;id_portfolio=' . $this->id_portfolio . '&amp;id_user=' . $this->id_user;
            }
    ...
    public function getAttachedFile($ext = false, $code = false)
    {
        ...
        while ($row = sql_fetch_row($files_info)) {
            if ($ext) {
                $link = 'index.php?modname=eportfolio&amp;type=ext&amp;op=downloadfile&amp;id_presentation=' . $this->id_presentation . '&amp;id_portfolio=' . $this->id_portfolio . '&amp;id_user=' . $this->id_user . '&amp;id_file=' . $row[MYFILE_ID_FILE] . '&amp;code=' . $code . '&amp;no_redirect=1';
            } else {
                $link = 'index.php?modname=eportfolio&amp;op=downloadfile&amp;id_portfolio=' . $this->id_portfolio . '&amp;id_user=' . $this->id_user . '&amp;id_file=' . $row[MYFILE_ID_FILE];
            }
