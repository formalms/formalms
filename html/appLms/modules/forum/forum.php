...
function forum()
{
    ...
    $re_last_post = sql_query("
		...
		FROM " . $GLOBALS['prefix_lms'] . "_forum AS f
		LEFT JOIN " . $GLOBALS['prefix_lms'] . "_forummessage AS m ON ( f.last_post = m.idMessage )
		LEFT JOIN " . $GLOBALS['prefix_lms'] . "_forumthread AS t ON ( m.idThread = t.idThread )
		WHERE f.idCourse = '" . (int)$_SESSION['idCourse'] . "' AND (t.privateThread=0 OR (t.privateThread=1 AND t.author='" . getLogUserId() . "') OR '1'='" . $mod_perm . "')");
    ...
    while (list($idF, $title, $descr, $num_thread, $num_post, $locked, $emoticons) = sql_fetch_row($re_forum)) {
        if (checkPerm('view', $idF) || checkPerm('mod', true)) {
            $c_css = '';
            $mess_notread = 0;
            $thread_notread = 0;
            // NOTES: status
            ...
            $content = array(
                $emoticon_img,
                '<a' . $c_css . ' href="index.php?modname=forum&amp;op=thread&amp;idForum=' . $idF . '"><strong>' . $status . ' ' . $title . '</strong></a>',
                $descr,
...
function insforum()
{
    ...
    list($id_forum) = sql_fetch_row(sql_query("SELECT LAST_INSERT_ID()"));
    if (!empty($recipients)) {
    ...
    if (!empty($recipients)) {
        ...
        if ($can_notify) {
            ...
            foreach ($recipients as $recipient) {
                ...
                if (!issetNotify('forum', $id_forum, $recipient)) {
                    $re = setNotify('forum', $id_forum, $recipient);
                }
            }
        }
    }
...
function thread()
{
    ...
    if (!checkPerm('view', (int)$_GET['idForum']))
    ...
    $moderate = checkPerm('moderate', $id_forum);
	...
	WHERE t.idForum = '" . $id_forum . "' AND (t.privateThread=0 OR (t.privateThread=1 AND t.author='" . getLogUserId() . "') OR '1'='" . $mod_perm . "')";
    ...
    while (list($idT, $t_author, $posted, $title, $num_post, $num_view, $locked, $erased, $important) = sql_fetch_row($re_thread)) {
        ...
        $content_temp = ($erased && !$mod_perm ?
            ...
            ($important ? '<span class="glyphicon glyphicon-warning-sign"></span>&nbsp;' : '') . ' <a' . $c_css . ' href="index.php?modname=forum&amp;op=message&amp;idThread=' . $idT . '"><strong>' . $title . '</strong></a>');
    ...
    if (!$locked_f && checkPerm('view', $id_forum)/*checkPerm('write', true)*/) {
        $text_inner .= '<a class="btn btn-default" href="index.php?modname=forum&amp;op=addthread&amp;idForum=' . $id_forum . '">
											<span class="glyphicon glyphicon-plus-sign"></span>&nbsp;
											<span>' . $lang->def('_ADDTHREAD') . '</span>
										</a>';
    }
    if ($can_notify) {
        $text_inner .= '<a class="btn btn-default" href="index.php?modname=forum&amp;op=thread&amp;notify=1&amp;idForum=' . $id_forum . '&amp;ini=' . $ini_page . '">
											<span class="glyphicon glyphicon-star"></span>&nbsp;
											<span>' . (!$is_notify ? $lang->def('_NOTIFY_ME_FORUM') : $lang->def('_UNNOTIFY_ME_FORUM')) . '</span>
										</a>';
    }
    if ($mod_perm) {
        $text_inner .= '<a class="btn btn-default" href="index.php?modname=forum&amp;op=modstatus&amp;idForum=' . $id_forum . '">
    ...
    if (!$locked_f && checkPerm('view', $id_forum)) {
        $text_inner .= '<li><a href="index.php?modname=forum&amp;op=addthread&amp;idForum=' . $id_forum . '">
											<span class="glyphicon glyphicon-plus-sign"></span>&nbsp;
											<span>' . $lang->def('_ADDTHREAD') . '</span>
										</a></li>';
    }
...
function addthread()
{
    ...
    $GLOBALS['page']->add(Form::getCheckbox($lang->def('_PRIVATE_THREAD'), 'private', 'private', 1, false), 'content');
...
function insthread()
{
    ...
    $private = importVar('private', true, '0');
    $ins_query = "
...
function message()
{
    ...
    $moderate = checkPerm('moderate', $id_forum);
    $mod_perm = checkPerm('mod', true);
    $write_perm = checkPerm('view', $id_forum);
    $acl_man =& Docebo::user()->getAclManager();
    ...
    while (list($id_message, $message_info) = each($messages)) {
        ...
        $sender = '<div class="forum_author">'
            ...
            . '<a href="index.php?modname=forum&amp;op=viewprofile&amp;idMessage=' . $id_message . '&amp;ini=' . $ini_page . '&amp;idThread=' . $id_thread . '">'
        ...
        if ($moderate || $mod_perm) {
            if ($message_info['locked']) {
                $action .= '<a class="btn btn-default" href="index.php?modname=forum&amp;op=moderatemessage&amp;idMessage=' . $id_message . '&amp;ini=' . $ini_page . ' title="' . $lang->def('_DEMODERATE') . ' : ' . strip_tags($message_info['title']) . '">
											<span class="glyphicon glyphicon-ban-circle"></span> &nbsp;
											<span>' . $lang->def('_DEMODERATE') . '</span>
										</a>';
            } else {
                $action .= '<a class="btn btn-default" href="index.php?modname=forum&amp;op=moderatemessage&amp;idMessage=' . $id_message . '&amp;ini=' . $ini_page . '" title="' . $lang->def('_MODERATE') . ' : ' . strip_tags($message_info['title']) . '">
											<span class="glyphicon glyphicon-ban-circle"></span> &nbsp;
        ...
        if (!$locked_t && !$locked_f && !$message_info['locked'] && $write_perm) {
            $action .= '<a class="btn btn-default" href="index.php?modname=forum&amp;op=addmessage&amp;idThread=' . $id_thread . '&amp;idMessage=' . $id_message . '&amp;ini=' . $ini_page . '" title="' . $lang->def('_REPLY') . ' : ' . strip_tags($message_info['title']) . '">
										<span class="glyphicon glyphicon-comment"></span> &nbsp;
										<span>' . $lang->def('_QUOTE') . '</span>
									</a>';
        }
        if ($moderate || $mod_perm || ($m_author == getLogUserId())) {
            $action .= '<a class="btn btn-default" href="index.php?modname=forum&amp;op=modmessage&amp;idMessage=' . $id_message . '&amp;ini=' . $ini_page . '" title="' . $lang->def('_MOD_MESSAGE') . ' : ' . strip_tags($message_info['title']) . '">
										<span class="glyphicon glyphicon-pencil"></span> &nbsp;
										<span>' . $lang->def('_MOD_MESSAGE') . '</span>
									</a>
									<a class="btn btn-default" href="index.php?modname=forum&amp;op=delmessage&amp;idMessage=' . $id_message . '&amp;ini=' . $ini_page . '" title="' . $lang->def('_DEL') . ' : ' . strip_tags($message_info['title']) . '">
										<span class="glyphicon glyphicon-remove"></span> &nbsp;
        ...
        if ($moderate || $mod_perm) {
            if ($message_info['locked']) {
                $action .= '<li><a href="index.php?modname=forum&amp;op=moderatemessage&amp;idMessage=' . $id_message . '&amp;ini=' . $ini_page . '">
											<span class="glyphicon glyphicon-ban-circle"></span> &nbsp;
											<span>' . $lang->def('_MODERATE') . '</span>
										</a></li>';
            } else {
        ...
        if (!$locked_t && !$locked_f && !$message_info['locked'] && $write_perm) {
            $action .= '<li><a href="index.php?modname=forum&amp;op=addmessage&amp;idThread=' . $id_thread . '&amp;idMessage=' . $id_message . '&amp;ini=' . $ini_page . '">
										<span class="glyphicon glyphicon-comment"></span> &nbsp;
										<span>' . $lang->def('_QUOTE') . '</span>
									</a></li>';
        }
        if ($moderate || $mod_perm || ($m_author == getLogUserId())) {
            $action .= '<li><a href="index.php?modname=forum&amp;op=modmessage&amp;idMessage=' . $id_message . '&amp;ini=' . $ini_page . '">
										<span class="glyphicon glyphicon-pencil"></span> &nbsp;
										<span>' . $lang->def('_MOD_MESSAGE') . '</span>
									</a></li>
									<li><a href="index.php?modname=forum&amp;op=delmessage&amp;idMessage=' . $id_message . '&amp;ini=' . $ini_page . '">
										<span class="glyphicon glyphicon-remove"></span> &nbsp;
										<span>' . $lang->def('_DEL') . '</span>
									</a></li>';
        }
    ...
    if (!$locked_t && !$locked_f && $write_perm) {
        $text_inner .= '<li><a href="index.php?modname=forum&amp;op=addmessage&amp;idThread=' . $id_thread . '&amp;ini=' . $ini_page . '">
											<span class="glyphicon glyphicon-plus-sign"></span>&nbsp;
											<span>' . $lang->def('_REPLY') . '</span>
										</a></li>';
    }
    ...
    if ($mod_perm) {
        $text_inner .= '<li><a href="index.php?modname=forum&amp;op=modstatusthread&amp;idThread=' . $id_thread . '&amp;ini=' . $ini_page . '">
										...
										<li><a href="index.php?modname=forum&amp;op=changeerased&amp;idThread=' . $id_thread . '&amp;ini=' . $ini_page . '">
											<span class="glyphicon glyphicon-' . ($erased_t ? 'lock' : 'ban-circle') . '"></span>&nbsp;
											<span>' . ($erased_t ? $lang->def('_UNERASE') : $lang->def('_MODERATE')) . '</span>
										</a></li>';
    }
    ...
    if (checkPerm('moderate', $id_forum) || checkPerm('mod', true)) {
        if ($is_important) {
            $text_inner .= '<li><a href="index.php?modname=forum&op=message&amp;idThread=' . $id_thread . '&amp;ini=' . $ini_page . '&amp;important=2">
												<span class="glyphicon glyphicon-warning-sign"></span>&nbsp;
												<span>' . $lang->def('_SET_NOT_IMPORTANT_THREAD') . '</span>
											</a></li>';
        } else {
            $text_inner .= '<li><a href="index.php?modname=forum&op=message&amp;idThread=' . $id_thread . '&amp;ini=' . $ini_page . '&amp;important=1">
												<span class="glyphicon glyphicon-warning-sign"></span>&nbsp;
												<span>' . $lang->def('_MARK_AS_IMPORTANT') . '</span>
											</a></li>';
        }
...
function forumsearch()
{
    ...
    while (list($id_f) = sql_fetch_row($re_forum)) {
        ...
        if (checkPerm('view', $id_f) || checkPerm('mod', true))
            $forums[] = $id_f;
    }
    if (empty($forums)) {
