...
class Associate_Question extends Question
{
    ...
    public function edit($back_test)
    {
        ...
        if (isset($_POST['add_question'])) {
            ...
            while (list($id_aa) = sql_fetch_row($re_answer_asso)) {
                $existent_associate[$id_aa] = 1;
            }
            ...
            for ($j = 0; $j < $num_answer; ++$j) {
                ...
                if ($content != '') {
                    if (isset($_POST['elem_b_id'][$j]) && ($_POST['elem_b_id'][$j] != 0)) {
                        ...
                        if (isset($existent_associate[$id_old_a])) {
                            unset($existent_associate[$id_old_a]);
                        }
                        ...
                        $lastInsertId = sql_fetch_row(sql_query('SELECT LAST_INSERT_ID()'));
                        $id_assigned[$j] = $lastInsertId[0];
                    }
                }
            }
            ...
            for ($i = 0; $i < $num_answer; ++$i) {
                ...
                for ($j = 0; $j < $num_answer; ++$j) {
                    $selected = '';
                    if (!empty($_POST['elem_b'][$j])) {
                        if ($questAnswers[$j]['is_correct'] === $_POST['elem_b_id'][$i]) {
                            $selected = 'selected';
                        }
                        $content_b .= '<option value="' . $j . '"' . $selected . '>' . stripslashes($_POST['elem_b'][$j]) . '</option>';
                    }
                }
    ...
    public function play($num_quest, $shuffle_answer = false, $id_track = 0, $freeze = false, $number_time = null)
    {
        ...
        if ($id_track != 0) {
            ...
            if (sql_num_rows($re_answer_do)) {
                ...
                $find_prev = true;
                while (list($id_a, $id_sel) = sql_fetch_row($re_answer_do)) {
                    $answer_do[$id_a] = $id_sel;
                }
            }
        }
     ...
     * @param float $try   if true the function return the effective point that will be assigned
     *
     ...
     * @author Fabio Pirovano (fabio@docebo.com)
     */
    public function setMaxScore($score, $try = false)
    {
        list($num_correct) = sql_fetch_row(sql_query('
		SELECT COUNT(*)
		FROM ' . $GLOBALS['prefix_lms'] . "_testquestanswer 
    ...
    public function userScore($id_track, $number_time = null)
    {
        ...
        $query .= ' ORDER BY number_time DESC';
