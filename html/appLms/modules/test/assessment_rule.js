...
YAHOO.util.Event.onDOMReady(function() {
  ...
  YAHOO.util.Event.addListener("main_form", "submit", function(e) {
    ...
    YAHOO.util.Dom.get("competences_list").value = YAHOO.lang.JSON.stringify(
      competence_arr
    );
    YAHOO.util.Dom.get("courses_list").value = YAHOO.lang.JSON.stringify(
      course_arr
    );
  });
...
YAHOO.util.Event.onDOMReady(function() {
  CompetenceAC = (function() {
    ...
    var oDS = new YAHOO.util.XHRDataSource(
      competence_ac_url + "&authentic_request=" + $("#authentic_request").val()
    );
    ...
    var oAC = new YAHOO.widget.AutoComplete(
      "input_add_competence",
      "box_autocomplete_competence",
      oDS
    );
    oAC.generateRequest = function(sQuery) {
      return "&results=20&query=" + sQuery;
    };
    //oAC.delimChar = [","];
  ...
  CourseAC = (function() {
    // Use a LocalDataSource
    var oDS = new YAHOO.util.XHRDataSource(
      course_ac_url + "&authentic_request=" + $("#authentic_request").val()
    );
    oDS.responseType = YAHOO.util.XHRDataSource.TYPE_JSON;
    oDS.responseSchema = {
      resultsList: "courses",
    ...
    oAC.formatResult = function(oResultData, sQuery, sResultMatch) {
      return (
        (oResultData.code_highlight != ""
          ? "[" + oResultData.code_highlight + "] "
          : "") + oResultData.name_highlight
      );
    };
    oAC.itemSelectEvent.subscribe(function(sType, oArgs) {
...
function drawCourseList() {
  ...
  for (i in course_arr) {
    list += "<li>";
    list +=
      '<div style="margin-left: 1em; display:-moz-inline-box;display:inline-block;">';
    list +=
      '<a id="del_course_' +
      course_arr[i]["id"] +
      '" class="ico-wt-sprite subs_del" href="" title="' +
      lang["remove_item"] +
      '"><span></span></a></div>';
...
function drawCompetenceList() {
  ...
  for (i in competence_arr) {
    ...
    list +=
      '<div style="margin-left: 1em; display:-moz-inline-box;display:inline-block;">';
    list +=
      '<a id="del_competence_' +
      competence_arr[i]["id"] +
      '" class="ico-wt-sprite subs_del" href="#" title="' +
      lang["remove_item"] +
      '"><span></span></a></div>';
    list +=
      '<div style="text-align: right; margin-right: 1em; display:-moz-inline-box;display:inline-block; width: 60px;">';
    if (competence_arr[i]["type"] == "score") {
      var score =
        competence_arr[i]["score"] != null ? competence_arr[i]["score"] : 0;
      list +=
        '<input id="competence_score_' +
        competence_arr[i]["id"] +
        '" type="text" size="2" style="width: 85%;" value="' +
        score +
        '" />';
