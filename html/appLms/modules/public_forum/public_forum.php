...
function thread()
{
    ...
    $idForum = Forma\lib\Get::gReq('idForum', DOTY_INT, 0);
...
function delthread()
{
    ...
    if (isset($_POST['undo'])) {
        if (Forma\lib\Get::req('search', DOTY_INT) == 1) {
            Util::jump_to('index.php?modname=public_forum&op=search&amp;ini=' . $ini);
        } else {
            Util::jump_to('index.php?modname=public_forum&op=thread&idForum=' . $id_forum);
        }
    ...
    if (!$confirm) {
        $confirm = Forma\lib\Get::req('confirm', DOTY_INT, 0);
    }
    if ($confirm) {
        $forum_query = '
		SELECT last_post
...
function delmessage()
{
    ...
    if (isset($_POST['confirm']) || Forma\lib\Get::req('confirm', DOTY_INT)) {
