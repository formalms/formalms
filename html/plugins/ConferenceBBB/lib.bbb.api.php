...
class BigBlueButton
{
    ...
    public function createMeetingArray($name, $meetingID, $welcomeString, $mPW, $aPW, $SALT, $URL, $logoutURL)
    {
        ...
        if ($xml) {
            if ($xml->meetingID) {
                return ['returncode' => $xml->returncode, 'message' => $xml->message, 'messageKey' => $xml->messageKey, 'meetingID' => $xml->meetingID, 'attendeePW' => $xml->attendeePW, 'moderatorPW' => $xml->moderatorPW, 'hasBeenForciblyEnded' => $xml->hasBeenForciblyEnded];
            } else {
                return ['returncode' => $xml->returncode, 'message' => $xml->message, 'messageKey' => $xml->messageKey];
            }
        } else {
            return null;
        }
    ...
    public function getMeetingInfoArray($meetingID, $modPW, $URL, $SALT)
    {
        ...
        if ($xml && $xml->returncode == 'SUCCESS' && $xml->messageKey == null) {//The meetings were returned
            ...
            return ['meetingID' => $xml->meetingID, 'moderatorPW' => $xml->moderatorPW, 'attendeePW' => $xml->attendeePW, 'hasBeenForciblyEnded' => $xml->hasBeenForciblyEnded, 'running' => $xml->running, 'startTime' => $xml->startTime, 'endTime' => $xml->endTime, 'participantCount' => $xml->participantCount, 'moderatorCount' => $xml->moderatorCount, 'attendees' => $xml->attendees];
    ...
    public function getMeetingsArray($URL, $SALT)
    {
        ...
        if ($xml && $xml->returncode == 'SUCCESS' && $xml->messageKey) {//The meetings were returned
            ...
            foreach ($xml->meetings->meeting as $meeting) {
                ...
                $meetings[] = ['meetingID' => $meeting->meetingID, 'moderatorPW' => $meeting->moderatorPW, 'attendeePW' => $meeting->attendeePW, 'hasBeenForciblyEnded' => $meeting->hasBeenForciblyEnded, 'running' => $meeting->running];
            }
    ...
    public function getUsersArray($meetingID, $modPW, $URL, $SALT)
    {
        ...
        if ($xml && $xml->returncode == 'SUCCESS' && $xml->messageKey == null) {//The meetings were returned
            ...
            foreach ($xml->attendees->attendee as $attendee) {
                $users[] = ['userID' => $attendee->userID, 'fullName' => $attendee->fullName, 'role' => $attendee->role];
            }
