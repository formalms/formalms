...
class Report extends \ReportPlugin
{
        ...
        &$classrooms_editions_info)
    {
        ...
        if (in_array('_INSCR', $filter_cols)) {
            $th2[] = ['style' => 'align-center', 'value' => $rg_lang->def('_USER_STATUS_SUBS')];
        }
        if (in_array('_MUSTBEGIN', $filter_cols)) {
            $th2[] = ['colspan' => ($show_percent ? 2 : 1), 'style' => 'align-center', 'value' => $rg_lang->def('_MUSTBEGIN')];
        }
        if (in_array('_USER_STATUS_BEGIN', $filter_cols)) {
            $th2[] = ['colspan' => ($show_percent ? 2 : 1), 'style' => 'align-center', 'value' => $rg_lang->def('_USER_STATUS_BEGIN')];
        }
        if (in_array('_COMPLETECOURSE', $filter_cols)) {
            $th2[] = ['colspan' => ($show_percent ? 2 : 1), 'style' => 'align-center', 'value' => $rg_lang->def('_COMPLETED')];
        }
        ...
        if (in_array('_INSCR', $filter_cols)) {
            $th3[] = ['style' => 'align-center', 'value' => $rg_lang->def('_NUM')];
        }
        if (in_array('_MUSTBEGIN', $filter_cols)) {
        ...
        foreach ($id_courses as $index => $course_info) {
            ...
            if (isset($num_iscr[$index])) {
                if (in_array('_INSCR', $filter_cols)) {
                    $trow[] = ['style' => 'img-cell', 'value' => $num_iscr[$index]];
                }
                ...
                if (in_array('_MUSTBEGIN', $filter_cols)) {
                    if (isset($num_nobegin[$index])) {
                        ...
                        $trow[] = ['style' => 'img-cell', 'value' => $num_nobegin[$index]];
                        if ($show_percent) {
