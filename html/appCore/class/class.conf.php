...
class Config_Framework
{
    ...
    public function _maskTemplateManager()
    {
        ...
        $html .= Form::getDropdown($lang->def('_TEMPL_USE_FIELD'),
                                    ...
                                    'templ_use_field',
                                    $drop_field,
                                    FormaLms\lib\Get::sett('templ_use_field'));

        $html .= Form::getButton('save_and_refresh', 'save_and_refresh', $lang->def('_SAVE_AND_REFRESH'));

        if (FormaLms\lib\Get::sett('templ_use_field') != 0) {
            $field_obj = &$field_man->getFieldInstance(FormaLms\lib\Get::sett('templ_use_field'));
            if ($field_obj === null) {
			...
			WHERE id_common = '" . FormaLms\lib\Get::sett('templ_use_field') . "'";
    ...
    public function _saveTemplateManager()
    {
        $re = true;
        if (!isset($_POST['template_selected'])) {
            return true;
        }
