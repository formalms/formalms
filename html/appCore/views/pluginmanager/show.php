...
echo '<br/>';
echo Form::getButton('', 'submit_upload', 'Upload', 'forma-button forma-button--primary', 'style="width: auto; padding-left: 10px; padding-right: 10px;"');
echo Form::closeForm();
?>
    ...
    var table = $('#plugins').FormaTable({
        ...
        columns: [  // definisco le colonne
            ...
            { title: '<?php echo Lang::t('_PLUGIN_SETTINGS', 'configuration'); ?>', width: '5%', sortable: false, searchable: false, render: function(data, type, row, meta) {
                if (row['plugin_id']){
                    return ' <a class="forma-button forma-button--primary"  href="index.php?r=adm/pluginmanager/showSettings'+'&plugin=' + row['name'] + '"><?php echo Lang::t('_PLUGIN_SETTINGS', 'configuration'); ?></a>';
                }
            }},
                ...
                if ($info['plugin_id']){
                    if ($info['core']==="0"){
                        if (!$info['version_error']){
                                ...
                                $install+='<a class="forma-button forma-button--primary"  href="javascript:askUninstall(\'index.php?r=adm/pluginmanager/uninstall'+'&plugin=' + $info['name'] + '\');"><?php echo Lang::t('_PLUGIN_UNINSTALL', 'configuration'); ?></a>';
                                if ($info['update']){
                                    $error="";
                                    <?php if (!class_exists('ZipArchive')) { ?>
                                        $error+= '<?php echo Lang::t('_PLUGIN_ERROR_NOT_ONLINE_UPDATE', 'configuration'); ?><br>';
                                    <?php } ?>
                                    $install+= ' <a class="forma-button forma-button--primary" title="' + $error + '" style="color: #006d07;" href="index.php?r=adm/pluginmanager/update' + '&plugin=' + $info['name'] + ($info['online'] ? "&online=true" : "") + '"><?php echo Lang::t('_PLUGIN_UPDATE', 'configuration'); ?></a>';
                                }
                        }else {
                            $errors+= '<a class="forma-button forma-button--primary" style="color: #c80014;" href="javascript:;"><?php echo Lang::t('_PLUGIN_ERROR_OLD_VERSION', 'configuration'); ?></a>';
                        }
                    }
                } else {
                    if (!$info['dependencies_unsatisfied']){
                        $install+='<a class="forma-button forma-button--primary"  href="index.php?r=adm/pluginmanager/install&plugin=' + $info['name'] + '"><?php echo Lang::t('_PLUGIN_INSTALL', 'configuration'); ?></a>';
                    } else {
                        $dependencies = "";
                    ...
                    if ($info['core']==="0"){
                        if (!$info['version_error']){
                           ...
                           if($info['unsuitable_forma']) {
                            ...
                            if ($info['active'] == '1'){
                                $activate =' <a class="forma-button forma-button--primary"  href="index.php?r=adm/pluginmanager/deactivate'+'&plugin=' + $info['name'] + '"><?php echo Lang::t('_PLUGIN_DEACTIVATE', 'configuration'); ?></a>';
                                ...
                                $activate =' <a class="forma-button forma-button--primary"  href="index.php?r=adm/pluginmanager/activate'+'&plugin=' + $info['name'] + '"><?php echo Lang::t('_PLUGIN_ACTIVATE', 'configuration'); ?></a>';
            ...
            if ($info['plugin_id']){
                return '' +
                    '<form>' +
                        ...
                        '<button style="display: inline-block; width: auto; margin-left: 10px; height: 28px; line-height: 20px; padding-left: 10px; padding-right:10px;" class="forma-button forma-button--primary" submit><?php echo Lang::t('_SET', 'configuration'); ?></button>' +
                    '</form>'+
                '';
            }
        }},
        ...
        ajax: {
            url: "<?php echo FormaLms\lib\Get::rel_path() . '/ajax.adm_server.php?r=pluginmanager/getTableData'; ?>",
            type: "GET"
        },
        order: [[ 0, "asc" ]],
    });
