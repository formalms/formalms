	...
	public function getCourses($params) {
		...
		foreach($course_list as $key=>$course_info) {
			...
			$output['course_info'][]=array(
				...
				'course_link'=>Get::site_url() . _folder_lms_ . "/index.php?modname=course&amp;op=aula&amp;idCourse={$row['idCourse']}",
				'img_course'=>Get::site_url() . _folder_files_ . '/' . _folder_lms_ . '/' . Get::sett('pathcourse') . $row['img_course'],
