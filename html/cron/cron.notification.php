...
function sendLoNotifications()
{
    ...
    try {
        ...
        foreach ($notifications as $notification) {
            ...
            if (stripos($notification->getLastSend(), '00') !== 0) {
                continue;
            }
            ...
            $query = "SELECT cu.idUser FROM %lms_courseuser cu INNER JOIN %adm_user u ON u.idst = cu.idUser WHERE u.valid = 1 AND cu.idCourse = " . $idCourse;
            $rs = sql_query($query);
            $userIds = [];
