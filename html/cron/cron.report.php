...
function getReportRecipients($id_rep)
{
	...
	while ($row = sql_fetch_assoc($res)) {
		...
		if (!empty($recipients_flat)) {
			$qry3 = "
				SELECT u.email, su.value FROM %adm_user AS u 
				LEFT JOIN %adm_setting_user su ON su.id_user = u.idst AND su.path_name = 'ui.language'
				WHERE u.idst IN (" . implode(',', $recipients_flat) . ") AND u.email <> '' AND u.valid = 1
			";
			$res3 = sql_query($qry3);
			while (list($email, $language) = sql_fetch_row($res3)) $output[] = ['email' => $email, 'language' => $language];
			...
			if ($re_report && sql_num_rows($re_report)) {
				...
				if ($report_size > $report_max_email_size * 1048576) {
					...
					foreach ($recipients as $recipient) {
						...
						$subject = str_replace('[name]', $row['filter_name'], Lang::t('_SCHEDULED_REPORT_SUBJECT_', 'email'));
						$body = str_replace('[report_url]', $report_url, Lang::t('_SCHEDULED_REPORT_BODY_', 'email'));
